...
async function getItems({
  ...
  try {
    const response = await api
      .get("/orders", { params: { page, limit, description, categoryId } })
      .then((res) => res.data);
...
export function useOrders({ page, limit, description, categoryId }: Props) {
  return useQuery({
    queryKey: ["orders", { page, limit, description, categoryId }],
